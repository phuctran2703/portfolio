...
export default function EducationSection() {
  return (
    <section id="education" className="py-12 scroll-mt-16 px-4 md:px-8 lg:px-10">
      <div className="w-full max-w-4xl mx-auto flex flex-col justify-center items-center">
        ...
        <div className="w-full">
          ...
          {education.map((edu, index) => (
            <div
              key={index}
              className="rounded-lg border border-gray-200 dark:border-gray-800 bg-card text-card-foreground shadow-sm transition-transform duration-300 hover:shadow-md hover:-translate-y-1 mb-6"
            >
        ...
        <div className="w-full">
          {certifications.map((cert, index) => (
            <div
              key={index}
              className={`w-full p-6 rounded-lg border border-gray-200 dark:border-gray-800 bg-card text-card-foreground shadow-sm transition-transform duration-300 hover:shadow-md hover:-translate-y-1 mb-4 ${
              ...
              <div className="flex flex-col sm:flex-row gap-4 sm:space-x-6">
                ...
                <div className="flex-1 flex flex-col sm:flex-row justify-between items-center sm:items-center gap-2">
                  <div className="text-center sm:text-left flex items-center">
                    <h3 className={`text-lg md:text-xl font-bold ${
                      cert.link ? 'text-blue-600 dark:text-blue-400 hover:underline' : ''
                    }`}>
                      {cert.title}
                    </h3>
                    {cert.link && (
